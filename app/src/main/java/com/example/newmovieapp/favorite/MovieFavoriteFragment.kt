...
import com.example.newmovieapp.adapter.FavoriteMovieAdapter
...
@AndroidEntryPoint
class MovieFavoriteFragment : Fragment(), MovieAdapter.OnItemClickListener {
    ...
    private fun getFavoriteMovies() {
        favoriteViewModel.getMovies().observe(viewLifecycleOwner, {
            ...
            if (it.isNotEmpty()) {
                ...
                binding.rvListFavoriteMovie.layoutManager = GridLayoutManager(requireContext(), 2)
    ...
    override fun onItemClick(movie: Movie?, view: View) {
        val bundle = Bundle()
        bundle.putBoolean(Const.IS_MOVIE, true)
